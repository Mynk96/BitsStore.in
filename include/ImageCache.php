...
class ImageCache {
    ...
    private function docroot_to_url($src = null) {
        ...
        if (empty($this->cached_image_url)) {
            $image_path = str_replace($_SERVER['DOCUMENT_ROOT'], '', SITE_ROOT.DS.'images'.DS.$src);
            $image_url =  $_SERVER['REQUEST_SCHEME']."://" . $_SERVER['HTTP_HOST'].DS. substr($image_path, 1);
        } else {
    ...
    private function set_cached_filename() {
        ...
        $this->cached_filename = basename($this->image_src).'.'. $this->file_extension;
    }
